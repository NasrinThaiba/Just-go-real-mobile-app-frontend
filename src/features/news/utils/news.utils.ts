...
export function filterNewsByLanguage(
  ...
  const filtered = news.filter((item) => item.language === language);
...
export function filterNewsByCategory(
  ...
  return language
    ? filterNewsByLanguage(categoryNews, language)
    : categoryNews;
}
...
export function getTrendingNews(
  ...
  return [...filterNewsByLanguage(news, language)].sort(
    (first, second) =>
      second.views + second.likes * 2 -
      (first.views + first.likes * 2),
  );
}
